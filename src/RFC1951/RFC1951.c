...
static void invalidData(BitReadState *in_pBitReadState, 
	SetjmpStream *in_out_pWriteStream)
{
	longjmp(*in_out_pWriteStream->mpJmpBuffer, ReadResultInvalidData);
}

void readDataNoCompression(BitReadState *in_pBitReadState, 
	SetjmpStream *in_out_pWriteStreamState)
{
	uint16_t len, nlen, currentByteIndex;
	uint8_t currentByte;
	ByteStreamInterface *pByteStreamReadInterface = (ByteStreamInterface *) in_pBitReadState->mpSuperByteStream;
	...
	*/
	...
	(*pByteStreamReadInterface->mpfRead)(pByteStreamReadInterface, &len, sizeof(len));
	(*pByteStreamReadInterface->mpfRead)(pByteStreamReadInterface, &nlen, sizeof(nlen));
	...
	{
		(*pByteStreamReadInterface->mpfRead)(pByteStreamReadInterface, &currentByte, 1);
		(*in_out_pWriteStreamState->mByteStreamInterface.mpfWrite)(in_out_pWriteStreamState, &currentByte, 1);
	}
}
...
ReadResult parseRFC1951(ByteStreamInterface *in_out_pReadStreamState, 
	ByteStreamInterface *in_out_pWriteStreamState)
{
	BitReadState bitReadState;

	SetjmpStream setjmpReadStreamState;
	SetjmpStream setjmpWriteStreamState;
	...
	assert(((ByteStreamInterface *) in_out_pReadStreamState)->mpfRead != NULL);
	assert(((ByteStreamInterface *) in_out_pWriteStreamState)->mpfWrite != NULL);

	setjmpStreamInit(&setjmpReadStreamState, &jmpBuf, ReadResultPrematureEndOfStream, 
		in_out_pReadStreamState);

	setjmpStreamInit(&setjmpWriteStreamState, &jmpBuf, ReadResultWriteError, 
		in_out_pWriteStreamState);

	// The = is correct here
	if (result = setjmp(jmpBuf))
		return (ReadResult) result;

	bitReadStateInit(&bitReadState, &setjmpReadStreamState);
	...
	do
	{
		...
		pfTypeFunctions[type](&bitReadState, &setjmpWriteStreamState);
	} while (!last);
